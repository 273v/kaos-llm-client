...
These standards apply to Python code in `kaos-llm-client`.

`kaos-llm-client` publishes the `kaos_llm_client` import package. This package publishes CLI entry point(s): `kaos-llm-client`, `kaos-llm-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-llm-client` publishes import package `kaos_llm_client`.
- Declare the public API in `kaos_llm_client.__all__` when the package has
...
- Names exported from `kaos_llm_client.__all__`.
...
- Put optional integrations in named extras. Public extras currently declared: `azure`.
