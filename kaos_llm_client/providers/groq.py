...
Thin subclass of ``OpenAICompatibleClient`` -- Groq's API follows the OpenAI
...
logger = get_logger("kaos_llm_client.providers.groq")


class GroqClient(OpenAICompatibleClient):
    """Groq API client.

    Extends ``OpenAICompatibleClient`` with Groq-specific base URL and API key
    ...
    def _get_api_key_from_settings(self) -> str:
        ...
        if key is None:
            raise KaosLLMAuthError(
                ...
                fix="Set KAOS_LLM_GROQ_API_KEY environment variable or pass api_key= "
        ...
        if not secret:
            raise KaosLLMAuthError(
                ...
                provider=self._provider_name,
                fix="Set KAOS_LLM_GROQ_API_KEY to a valid API key.",
            )
        return secret
